...
#include "STM32_WifiESP_Alexa.h" // Fichier d'en-tête principal du driver
#include "udp_ssdp.h"			 // Inclure pour le handler UDP
...
const ESP01_WifiMode_t g_wifi_mode = ESP01_WIFI_MODE_AP;	  // ou ESP01_WIFI_MODE_STA selon ton mode voulu

// ==================== VARIABLES INTERNES ====================
UART_HandleTypeDef *g_esp_uart = NULL;				  // UART principal pour l'ESP01 (exposé pour UDP)
UART_HandleTypeDef *g_debug_uart = NULL;			  // UART pour le debug (exposé pour UDP)
 ...
 */
bool _accumulate_and_search(const char *pattern, uint32_t timeout_ms, bool clear_first)
{
	uint32_t start_tick = HAL_GetTick(); // Début du timeout
	char temp_buf[ESP01_TMP_BUF_SIZE];	 // Buffer temporaire pour les nouveaux octets
...
{
	char temp_buf[ESP01_TMP_BUF_SIZE];	 // Buffer temporaire pour vider les octets
...
{
	char cmd[ESP01_MAX_CMD_BUF], resp[ESP01_MAX_CMD_BUF * 2];					// Buffers pour la commande AT et la réponse
										...
										const char *body, size_t body_len)
{
	...
	char dbg[ESP01_SMALL_BUF_SIZE];								// Buffer debug
...
ESP01_Status_t esp01_send_404_response(int conn_id)
{
	...
	const char *body = ESP01_HTTP_404_BODY;											// Corps HTML 404
...
ESP01_Status_t esp01_get_connection_status(void)
{
	...
	char response[ESP01_DMA_RX_BUF_SIZE];																						// Buffer pour la réponse AT
...
ESP01_Status_t esp01_print_connection_status(void)
{
	...
	char msg[ESP01_MAX_DBG_BUF];														 // Buffer pour messages
	...
	char ip[ESP01_IP_BUF_SIZE];							  // Buffer pour l'IP
...
void esp01_process_requests(void)
{
	...
	while (keep_processing)		 // Boucle de traitement
	{
		...
		case PARSE_STATE_READING_PAYLOAD: // Lecture du payload HTTP
		{
			...
			// colon_pos points to the ':', payload starts at colon_pos + 1
			char *ipd_payload_start = colon_pos + 1;

			// Calculate the total expected frame length: length of header part + 1 (for colon) + payload length
			int expected_frame_len = (colon_pos - g_accumulator) + 1 + ipd_payload_len;

			// Check if the accumulator contains the full expected frame
			if (g_acc_len < expected_frame_len)
			{
			...
			if (idx != -1)
			{													  // Si un slot a été trouvé ou créé
				...
				if (req.has_ip)									  // Si l'IP est connue
				{
					...
					g_connections[idx].client_ip[ESP01_MAX_IP_LEN - 1] = '\0';				// Ensure null termination
					char dbg[ESP01_MAX_DBG_BUF];
				...
				char dbg[ESP01_MAX_DBG_BUF];															// Buffer debug
				snprintf(dbg, sizeof(dbg), "[DEBUG] Port client : %u", g_connections[idx].server_port); // Message debug
				_esp_logln(dbg);																		// Log debug
			}

			// Determine if it's an HTTP request or other data (like UDP)
			bool is_http = false;
			// Check the first few bytes of the payload to see if it looks like an HTTP request
			if (ipd_payload_len > 4) // Minimum length for "GET " or "POST"
			{						 // Minimum length for "GET " or "POST"
				if (strncmp(ipd_payload_start, "GET ", 4) == 0 ||
					strncmp(ipd_payload_start, "POST ", 5) == 0 ||
					strncmp(ipd_payload_start, "PUT ", 4) == 0 ||
					strncmp(colon_pos, "DELETE ", 7) == 0 ||
					strncmp(colon_pos, "HEAD ", 5) == 0)
				{
					is_http = true;
				}
			}
			...
			if (is_http)
			{
				...
				// Pass the payload start to the HTTP parser
				if (esp01_parse_http_request(ipd_payload_start, &parsed) == ESP01_OK && parsed.is_valid)
				{
					// --- Parsing des headers HTTP ---
					const char *first_line_end = strstr(ipd_payload_start, "\r\n"); // Cherche la fin de la première ligne
					if (first_line_end)												// Si trouvée
					{
						const char *headers_start = first_line_end + 2;				 // Début des headers
						const char *headers_end = strstr(headers_start, "\r\n\r\n"); // Fin des headers within payload
					...
					else
					{
						...
						if (strcmp(parsed.path, ESP01_HTTP_FAVICON_PATH) == 0) // Si favicon
						{
					...
					// Gère le body si nécessaire (POST/PUT) - assuming body is part of the current frame
					char *headers_end_body = strstr(ipd_payload_start, "\r\n\r\n");
					int headers_len_body = headers_end_body ? (headers_end_body + 4 - colon_pos) : 0;
					int body_len = ipd_payload_len - headers_len_body;
					if ((strcmp(parsed.method, "POST") == 0 || strcmp(parsed.method, "PUT") == 0) && body_len > 0)
					{
						// Body is present in the accumulator after headers_end_body
						// We don't process it here, but it will be removed with the rest of the frame.
					}
				}
				else
				{
					// HTTP parse error - discard the frame
					_esp_logln("[HTTP] HTTP parse error, discarding frame.");
					// The frame removal logic below will handle discarding.
				}
			}
			else
			{
				// Not a standard HTTP method, assume it's a raw UDP packet (like SSDP)
				_esp_logln("[UDP] Potential UDP packet detected."); // Log debug
				// The payload starts at ipd_payload_start
				// We need to pass this payload to a UDP handler
				// The length is ipd_payload_len
				// Note: For UDP, the client_ip and client_port from the +IPD header
				// are the source of the UDP packet, which is what we need for SSDP responses.
				// For TCP, these are the client's IP and port.

				// Pass source IP and port from the parsed request info
				udp_ssdp_handle_packet(req.conn_id, req.client_ip, req.client_port, colon_pos, ipd_payload_len);
			}

			// Remove the processed frame from the accumulator
			int total_to_remove = (colon_pos - g_accumulator) + ipd_payload_len; // Calculate size to remove
			if (g_acc_len >= total_to_remove)
			{																						  // Ensure we don't remove more than we have
				memmove(g_accumulator, g_accumulator + total_to_remove, g_acc_len - total_to_remove); // Shift remaining data
				g_acc_len -= total_to_remove;														  // Update length
				g_accumulator[g_acc_len] = '\0';													  // Null-terminate
				g_parse_state = PARSE_STATE_SEARCHING_IPD;											  // Reset state for next frame
																									  // keep_processing remains true if g_acc_len > 0 after removal
			}
			else
			{
				// Should not happen if ipd_payload_len was correct, but as a safeguard:
				char err_msg[ESP01_MAX_DBG_BUF];
				snprintf(err_msg, sizeof(err_msg), "[ERROR] Accumulator length (%u) less than expected frame size (%u)!", g_acc_len, total_to_remove);
				_esp_logln(err_msg); // Log error with more info
				g_acc_len = 0;
				g_accumulator[0] = '\0';
				g_parse_state = PARSE_STATE_SEARCHING_IPD;
				keep_processing = false; // Stop processing this cycle
			}
			break; // Sort du switch
		}
...
ESP01_Status_t esp01_get_current_ip(char *ip_buffer, size_t buffer_size)
{
	...
	if (!ip_line)									   // Si toujours pas trouvé
...
ESP01_Status_t esp01_get_at_version(char *version_buf, size_t buf_size)
{
	...
	char response[ESP01_DMA_RX_BUF_SIZE] = {0};																			  // Buffer pour la version AT
	ESP01_Status_t status = esp01_send_raw_command_dma("AT+GMR", response, sizeof(response), "OK", ESP01_TIMEOUT_MEDIUM); // Envoie la commande AT+GMR
